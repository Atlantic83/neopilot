...
- **`.neopilot/state.js`** — the truth, and the only thing you ever write. You read it on resume; the user never opens it.
- **`.neopilot/dashboard.html`** — the only human view. Copied from the template once and **never touched again**. No build step, no dependencies, nothing to generate: in a real browser it opens by double-click, and in an in-app pane it needs one static file server and no more (§3).
...
They cannot drift apart in opposite directions, because they are not equals: `state.js` is written on every edit, the snapshot only by `python3 .neopilot/sync.py`, and that same call copies the file into the page. So the snapshot is either equal to the file or older than it, and the page prefers whichever loaded last — the file, wherever it can be loaded. A forgotten `sync.py` costs a stale snapshot, never a wrong one, and never a blank screen.
...
`tier` is `null` until Phase 4 decides it. `polish` stays `null` unless the polish parameter was requested — see `phases/polish.md`, and do not add the block here on speculation.
...
G=$ROOT/.gitignore
git -C "$ROOT" rev-parse --git-dir >/dev/null 2>&1 && ! grep -qs '^\.neopilot/serve\.' "$G" && {
  [ -s "$G" ] && [ -n "$(tail -c1 "$G")" ] && printf '\n' >> "$G"   # a file without a trailing \n glues the lines together
  printf '.neopilot/serve.*\n' >> "$G"
}
python3 "$ROOT/.neopilot/sync.py"      # snapshot into the page + server if not up
```

**That one line is the whole of it, and what it prints is the address.** `sync.py` reuses the port recorded in `serve.pid` whenever it is free — and when that pointer is gone or dead, adopts a live server still listed in `serve.pids` before it ever rolls a new port — so the link the user already copied keeps working across restarts, and it starts the server detached from your session, so the end of a session no longer takes the dashboard down with it. It kills only processes that serve back **this run's** `serve.pids` — proof by content, not by process listing, so it holds where `ps` cannot see or does not exist: the machine-wide pid file that once handed a second project the first one's port cost a whole flight (2026-08-18), and a sweep of "every `http.server` except a whitelist" killed a live run's server mid-flight (2026-08-19). Never write either.
...
open "$A/dashboard.html" 2>/dev/null \
  ...
  || echo "open manually: $A/dashboard.html"
```
...
- **The pane's address is whatever the last `sync.py` line printed — nothing else.** If a later call's output carries "link moved", the port the pane sits on is dead: `navigate` it to the new one and re-say the address in chat. A pane left behind shows a dead page while the chat names a live one — that split is the "printed one address, opened another" failure.
...
- **One static server, and only for the ticking.** `python3 -m http.server` over `.neopilot`, bound to `127.0.0.1` and nothing wider, raised by `sync.py` when it is missing and stopped once the page has taken the final state — a landed run's `sync.py --kill` waits for that fetch in `serve.log` (`phases/8-final.md`). Its death is no longer an outage: the page keeps showing its snapshot, so never announce one, and never restart it by hand — the next `sync.py` does it. It serves the run's own directory — briefs, tickets, manifest — so binding it to `0.0.0.0` would publish them to the network. No build step, no bundler, no second page to maintain — `index.html` is a symlink onto the same file: the dashboard stays one static page that a browser can also open directly.
...
> Dashboard opened — `.neopilot/dashboard.html`, it refreshes itself.
...
| When | What |
|---|---|
| entering a phase | `python3 .neopilot/sync.py --enter <stage>` — **before the phase's opening move**, and every phase file now opens with its own `--enter` line; the command sets `active` + `startedAt` itself. **The one you left closes itself** — see below. A stage entered *after* its work records zero minutes, reads on the dashboard as skipped, and `sync.py` flags the transition |
...
Every one of them is one call, not an edit: `python3 .neopilot/sync.py --set <path> <value>`. Stages and tickets are addressed **by id** — `tickets.04.status in-progress`, `stages.spec.note "5 questions"` — never by position. The value is JSON where it parses (`3`, `true`, `{"passed":10,"failed":0}`), the word `now` writes the current timestamp, and anything else lands as a string. `--add <path> <value>` appends to a list (`debt.assumptions`, `additions`), `--inc <path>` bumps a counter (`retries`, `handoffs`). Status transitions stamp their own edges: `in-progress` writes `startedAt`, `done` writes `finishedAt`, `repair` bumps `repairs` — a half-written transition is impossible because the write is one. Every such call moves `updatedAt` itself. Hand-editing the file still works — it is plain JSON — but it is the version of the ritual that can be half-done: a stale `old_string`, a missed pulse, a `startedAt` nobody wrote. The setter cannot do any of those. Either way the screen follows within ten seconds wherever it is open, the state is proved to parse, the snapshot catches up, and a server that died comes back on its old address.

**A transition is a command, not a note about one — `python3 .neopilot/sync.py --enter <stage>` runs *before* the phase's opening move, and `sync.py` closes what the run has passed.** The order is the hard rule: the stage you left is closed at the `startedAt` written *now*, so work begun ahead of the mark is billed to it and no later edit returns those minutes. Underneath sits the same invariant as before — an invariant, not an event: nothing earlier than the active stage may also be active. So the stage you left is closed at the moment the next one opened — the very timestamp you would have written by hand — and the lines it prints with `·` say which. The one exception is in the model of the work itself: **review does not close build**, because reviews run per ticket inside the build; anything later closes both. `skipped` and `failed` are never touched — turning a conscious state into `done` would erase what was said about the run.

This is here because on 2026-08-19 `spec` stayed `active` for two and a half hours beside a finished plan and a running build, and on 2026-09-22 the briefing's questions were billed to the manifest for the same reason — and the person who noticed both times was the user, looking at the dashboard. Half a ritual performed by hand is a ritual that will be half-performed, which is why the mark is now a command and not a reminder.

**Lines starting with `!` are the ones left for you, and they are fixed in the same turn.** What cannot be derived is not guessed: a stage the run walked past but never marked — `pending` behind an active one — needs `skipped` **and a reason**, and only you know it. Same for a ticket in flight with no `startedAt`. A wrong guess about what you meant is worse than the inconsistency it would paper over. The audit's reach is the whole state, not just transitions: it also flags a `skipped` stage with no `note`, a `done` stage whose artifact (`manifest.md`, `spec.md`, the brief file, `interfaces.md`) is not on disk, a spec closed with `coverage` still `null`, tickets flying while `build` was never opened, a `blockedBy` pointing nowhere, at itself, or in a cycle, `repairs`/`retries`/`handoffs` past the ceiling of two, a `finishedAt` earlier than its `startedAt`, any mark later than the run's own `finishedAt`, and a `final` stage closed with `finishedAt` or `blind` still unwritten — or written without its `checked`/`matched` counts.

**The same audit lives inside the dashboard.** The page runs its own copy of these checks on every poll and shows the findings as a "State check" strip under the documents row — a contradiction is visible within ten seconds of hitting the file, not whenever someone next remembered to call `sync.py`. That closes the one gap the console audit could never reach: a run where `sync.py` was simply never called. What the page cannot check stays console-side — the artifact-existence rules (`manifest.md` on disk, `spec.md`, the brief file, `interfaces.md`) need a filesystem the browser does not have, so those lines only ever appear in the console output.
...
- **`updatedAt` moves on every write.** The dashboard shows "updated N ago" from it and turns the line warning-coloured when the silence runs long — that is the user's only way to tell "work in progress" from "agent died". The template knows that a ticket in flight means no writes for tens of minutes and holds the warning back until three quarters of an hour; between tickets it goes back to five. So the warning means what it says, and you do not need to invent keep-alive writes to silence it.
