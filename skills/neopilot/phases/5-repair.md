...
**First: not every finding comes here.** Only what the reviewer put in `BLOCKING` — a requirement not delivered, a spec decision missing or implemented differently without an amendment, an invented fact about the user, unrequested surface, silent narrowing of an acceptance criterion, a red suite, a defect proven by a trace, a secret's value in the diff, a test bound to what the next ticket may rewrite — becomes a follow-up. Craft judgement calls go to `state.js` under `concerns` and are triaged once, at the end, by the whole-project pass (`phases/6-review.md`, `phases/8-final.md`). Sending every finding down this path is what makes reviewing everything feel unaffordable: thirteen repairs across nine tickets, each adding about forty percent to its ticket's clock.

A ticket comes back imperfect in two very different ways, and telling them apart is the whole of this section:

- **Incompletion** — a red test, a blocking review finding, an acceptance criterion met in letter and dodged in substance. The executor *could* have done it and did not.
- **Failure** — `BLOCKED`, `NEEDS_CONTEXT`, or a repair that has already failed. The executor tried and could not.

| | Incompletion | Failure |
|---|---|---|
| Goes to | **the same subagent**, by message, its context intact — a **follow-up** | a **fresh context**, and only with a changed approach |
...
**A follow-up costs one line.** Do not resend `interfaces.md`, the spec sections or the testing contract — it has seen all three. Send the condition:

```
Test `parses empty address` is red:
<last 10 lines of output>

Name the cause in one line, then fix it — so the test passes.
Touch nothing else. Return the contract again.
```

- **The cause first, then the fix — one line of it, in the follow-up.** A repair that starts at the symptom fixes the symptom: the same defect returns two tickets later through a different door, and by then the `repairs` counter is spent. If the executor cannot name a cause, that is `BLOCKED`, not a workaround. **A weakened or deleted assertion is not a repair** — the re-review sees the fix's diff and this is the first thing it is looking at.
- **Two follow-ups into one context, then it stops being the cheap option.** By the third the context is no longer the fresh one that made this worth doing, and the repair moves to the right-hand column: new context, changed approach. That is the same rule as for a failed ticket, because by then it is one.
- **State the finding as a condition, never as "fix it up".** "Make it better" is an invitation to rewrite what already worked. Every repair names something checkable: this test green, this field visible, this error handled.
...
Retry **once**, in a fresh context, with the error attached and the failing test named — and bump `retries` in `state.js` (`--inc tickets.NN.retries`) the same way `repair` bumps `repairs`. A restart the counter never saw is a restart the ceiling cannot stop. If that fails too, one further attempt is allowed **only with a changed approach** — a different design decision, a different library, a path the ticket now names explicitly. Running the same attempt again with more hope is not a retry, and it is the only version of this that is forbidden.
...
## When the subagent never answers

Every path above starts with a return — `BLOCKED`, a red suite, a finding. There is a quieter failure: the call itself dies. The spawn errors out, the context is lost mid-ticket, or the return arrives with no contract block and a one-line ask produces none either. None of that is a status the executor can send — it is the absence of one, and it is not `in-progress`.

- **A ticket is not left `in-progress` with nobody behind it.** The dashboard clock running on a dead context is the same lie as `pending` with work marks. Either the retry is already flying, or the status moves back — to `pending` if it will be retried, to `failed` if this was the second attempt.
- **It spends `retries`, not `repairs` and not `handoffs`.** The dead context is unreachable by definition, so the follow-up column does not apply: fresh context, `--inc tickets.NN.retries`, same ceiling of two. Handoff files, if any exist, go to the successor exactly as they would after a `HANDOFF` return.
- **What the dead context wrote stays written.** `git status`, then the full suite, before the retry launches: uncommitted work is the successor's starting material, and a red tree is named in its prompt — the same rule as on a resume (`phases/0-preflight.md`).
- **How long a call may run before it counts as dead is the harness's business, not this file's.** What is yours is the moment after: once the call is over without a contract, the state stops pretending the agent still exists. A ticket that sits `in-progress` forever is not patience — it is the one failure shape the dashboard reports as health.
...
4. **Tell the user one line, in every mode including full:** "The schema from the plan couldn't hold two addresses per request — I adjusted it, the requirement is the same". They do not need the reasoning. They do need to know the plan moved, because a plan that moves silently is how the final report and their memory of the project stop matching.
