...
Not a phase in sequence. It runs **inside Phase 8, between the blind acceptance and the final report**, and it is the only part of NeoPilot that repeats itself deliberately.
...
```
Polish asked for a benchmark — something to compare against. There is none in
the task and nowhere to get one, so there would be nothing to compare with:
skipping, report below.
```

`reference.md` is collected in Phase 2 (`phases/2-briefing.md`). If the parameter was switched on mid-run, after the briefing, collect it now: **one question, in every mode including full.** The user asked for polish explicitly, so the question that makes polish possible is not an extra question — it is the request being carried out.
...
- `.neopilot/<dir>/<date>-brief.md` — the user's own words
- `.neopilot/<dir>/reference.md` — the comparables
...
> Attached is the brief — what was ordered — and the benchmark — what it should
> be like. Launch the project and walk through it the way the customer would.
>
> Put the result next to the benchmark and name the concrete differences. Not
> a verdict of "worse/better", but differences: what exactly differs and what
> must change for that difference to disappear.
>
> Each finding — one sentence, as a checkable condition: what must become
> true. "Doesn't look premium enough" is not a finding. "Heading and body
> text set at the same size; in the benchmark they differ twofold" is a finding.
>
> Fix nothing yourself. Propose no new capabilities: you are comparing
> against the benchmark, not inventing a product. If there are no differences —
> say so, that's a normal answer.
>
> No more than 12 findings, sorted by how visible the difference is.
...
- A finding whose fix would touch a requirement — anything that changes *what the product does* rather than how it looks or reads — is not a polish ticket. It is either an `A##` with a parent or a question, per the depth rules in `phases/3-spec.md`.

**4. The books.** Append a round to `state.js` → `polish` (shape below), tell the user one plain line, and go to the next round.

> Round 2: found 4 differences from the benchmark, fixed 3 — typography, spacing, empty-list state.
...
3. **The user says stop.** In any form, at any point, including "enough", "that's sufficient", "leave it as is". Ends the loop immediately, mid-round if need be.
...
If, after a round's tickets are in, the full suite is red, or the blind acceptance's verdict on any requirement has moved backwards — `implemented` that is now `partial` — revert that round's commits, record it, and stop the loop. Do not spend a round fixing a round.
...
Compared against the benchmark — <what exactly>. Three rounds, 8 differences fixed.

| What got better | Round |
|---|---|
| Typography: headings and body text now differ in size, as in the benchmark | 1 |
| The empty list is no longer a blank screen — there's a hint of what to do | 2 |

Differences I left untouched:

| Difference | Why |
|---|---|
| Product photos | need yours — currently visible placeholders |
```

Two rules for it. **What the loop declined to fix is listed, not omitted** — a polish section that shows only wins is an advertisement. And **a round that was rolled back appears with its reason**, in the same plain language as everything else: "The fourth round broke checkout — rolled back, left as it was".
