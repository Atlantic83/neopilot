...
**First move — `python3 .neopilot/sync.py --enter briefing`, before any work below.** A stage entered after its work records zero minutes and reads on the dashboard as skipped.
...
|---|---|
| **1. The flop** | The project is delivered, it works, and nobody uses it. What happened? |
| **2. The collision** | Which two requirements cannot both be true? Not inside one — between different manifest rows |
| **3. The unverified** | What does the user consider solved that isn't? "People will use this", "the data will come from somewhere", "I have an audience" |
| **4. The price** | Which requirement will eat half the build for a small share of the value — and does the user know? |
| **5. The condition** | What's not in the brief but without which the result is pointless? Not an implied requirement (that's `R##i`) — a success condition: who will fill it, who will maintain it, where the first data comes from |
| **6. The second week** | What happens when this stops being new: growth, duplicates, moderation, support, other people's hands |
| **7. The second actor** | Who else will use this besides the one described? The brief usually has not a line about them |
...
And it is **a grilling of the task, not of the person.** "Requests will come in — who reads them on Saturday?" is the pass working. "Are you sure anyone needs this at all?" is not a finding, it is an opinion, and it buys nothing that can be built.
...
**Recommend an answer with every question.** "Save requests into a Google Sheet or straight into a database? I'd take the sheet — you can see it and no server is needed." The user can accept in one word. Never ask an open question where a recommended default would do.
...
- **Nothing left open? Say so and go.** "No questions — everything in the task is unambiguous, writing the specification." Mark the `briefing` stage `skipped` in `state.js` with the note "no questions needed", so the user sees a decision rather than a step that quietly did not happen.
...
Say once why there will be many — "the task is big and a lot is undefined, there will be more questions than usual" — and then just work. **Do not apologise for the count as you go**: a mode the user chose does not need excusing, and "sorry, one more question" every third turn teaches them they picked wrong.
...
5. **Untestable requirements** — "beautiful", "convenient", "fast". Turn one into something checkable, or accept it as a matter of taste and record that. Do not spend three questions here.
   ...
   The cheapest way to turn one into something checkable is to ask what it should be **like**: "Are there sites this should resemble? Send two or three." An answer here is worth more than three rounds of guessing what "modern" meant — see *Benchmark* below.
...
## Benchmark — what the result should be like

The manifest records *what* to build. Nothing yet records **what it should be like**, and for anything with a surface — a page, an app, a piece of copy — that gap is where "built to requirements but looks wrong" comes from. The brief almost never says it, and the user almost always has it in their head.

So: **whatever the user hands you that a finished result could be measured against goes into `.neopilot/<dir>/reference.md`.** Reference sites, a competitor, screenshots, a text whose tone they like, a number they want beaten, "like the bank's app". One line each, verbatim after redaction, with where it came from.
...
## Appearance
- linear.app — "that kind of clean, nothing extra" (answer to question 3)
- the screenshot in the brief — the card layout

## How it should feel
- "a request goes through in two clicks, no sign-up"

## Copy
- a sample email the user sent — short, casual tone

## What it must not be
- "I don't want it like a government portal"
```
...
- **A comparable is not a requirement.** It never becomes an `R##`, never gets a status, never gates anything. "Like linear.app" is a direction; "must have a dark theme" is a requirement, and if the user said that, it belongs in the manifest.
...
- Answer is "don't know" → `placeholder`, and the build gets a stub with a visible label.

And every answer that **cancels, adds or reverses** something is also appended to the brief file under `## Additions` — dated, verbatim, per `phases/1-manifest.md` §2. Not instead of the manifest row: as well as it. The row records what happened to a requirement; the brief records what was asked for, and the two independent checks are given only the second one.

## When the task changes after this phase

The interview ends; the user does not. "Remove SMS" at ticket four, "and add export too" while a review is running — this is ordinary, not a failure of the briefing, and it has one procedure wherever in the flight it arrives:

1. **The brief file first** — their words, verbatim and dated, under `## Additions`. First because it is the step that gets skipped: updating the manifest feels like having recorded the change, and the manifest is the one file the gates may not read.
...
3. **The plan** — a new `G##` becomes a ticket, a `deferred` row, or a line in the report, decided exactly as it would have been at that point in the run (`phases/4-plan.md`). Say which, in one line, and say what it does to everything else: "Taking it, but the landing page slips then". A requirement accepted silently mid-flight is a schedule the user never agreed to.
...
**Facts about the user are not yours to invent.** Their prices, texts, addresses, business rules, accounts, brand colours. These become `placeholder` in the manifest, visibly labelled filler in the code (`[PRICE — fill in]`, not `$99`), and a line in the final report. A plausible invented price is worse than an obvious blank: the blank gets fixed, the price gets shipped.
...
Identical here — the two modes part company only at the spec and the plan, two phases later. Same rules as `semi`, no cap, and the wider line on which forks reach the user. Keep going while genuine forks remain, then say so plainly: "No more questions, writing the specification".

The thing to guard against in these two is not length, it is **drift into process**. "Which stack do we take?", "cut the tasks smaller?", "show the spec?" are not interview questions — they are the questions NeoPilot exists to answer itself, and asking them is how a mode the user chose for its thoroughness turns into the meeting they were avoiding. Every question still names its manifest row.

## Closing

Before leaving this phase, check **gate G1**: every manifest row has a status; nothing is `open` without a recorded reason. Then open the next stage — `python3 .neopilot/sync.py --enter spec` — announce the transition in one line — "Got it. Writing the specification" — and go to Phase 3.

**Close the stage the way it ran.** Questions asked → `done` with the count as its note ("3 questions"). Forks settled without asking, in the modes that let you → also `done`, "N decisions settled without asking". `skipped` with "no questions needed" marks only the phase that asked nothing **and** settled nothing: a stage filed as "not needed" while its answers sit in the manifest and the brief's Additions is a lie the user will find.
