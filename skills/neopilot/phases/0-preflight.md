...
|---|---|---|
| no `.neopilot/` | **new repo** | everything below, in order |
| `.neopilot/state.js` with `finishedAt: null` | **resume** | step 3a only — then *Resuming an interrupted flight* at the end of this file |
| `.neopilot/` exists, last run has `finishedAt` set | **new feature in a configured repo** | steps 1, 3, 3a, 5, 7 only |
| the same, but `updatedAt` is **under five minutes old** and a live server is on this `.neopilot/` | **the run is going on in another window** | none — say so and ask which window carries on |
...
**Nothing here is a question for the user.** These are process decisions, not product ones. No mode buys the user a say in where ticket files live; asking about it is exactly the kind of question NeoPilot exists to remove.
...
- `.neopilot/` — a previous run? Then this is a **resume**, see below.
...
The brief carries **the date it was dictated in its filename** — `2026-08-07-brief.md` — even though the directory now carries a date too, and the two are not the same date. The directory's is the day the flight started; the brief's is the day *this* brief was dictated. A run directory outlives one conversation: the user comes back a month later with "finish it", a second brief gets appended, and a file called `brief.md` gives no way to tell which sitting is which.

**Then add the run to the register** — one row in `.neopilot/README.md`, under `## Runs`:

```markdown
| 2026-08-07 | `2026-08-07-telegram-repair-bot--wip` | in progress | — |
```

Newest at the bottom, so the table reads in the order the runs happened. Phase 8 closes the row (`phases/8-final.md`); until then it says "in progress" and nothing else, because there is nothing true to say yet.

**On a new repo there is no README yet — step 4 writes it, and its template already carries this row.** Append here only when the file exists, which is every flight after the first: that case skips step 4, which *writes* the README, but not this, which *appends* to it — and skipping both is how a configured repo ends up with a register that stopped at the first run. If the file exists but has no `## Runs` section (a repo configured before 2026-08-19), add the heading and the table header above the row, and leave the rest of the README untouched.
...
Write `.neopilot/README.md` — a short note for the human, not for the agent:

```markdown
# How to read this folder

- `dashboard.html` — opens by itself at the start of the build; a double-click works too.
  Stages, progress, time, what's left. It refreshes itself while the build runs.
  `index.html` next to it — the same file under a different name, so the dashboard
  opens at the short address. No need to open it separately.
- `<date>-<project>/` — the folder of one build. The date in the name is the day
  the build started, so the folders sort top to bottom in order. The `--wip`
  suffix means "still in progress"; when the build is delivered, the suffix
  comes off. Inside:
  - `<date>-brief.md` — your original task, word for word, and "Additions" —
    everything you said later. The task text itself is never rewritten.
  - `manifest.md` — the list of requirements and what became of each.
  - `spec.md` — the specification.
  - `tickets/` — the tasks the build is cut into (on a small build there are none).

If the build was interrupted — tell the agent "resume neopilot", it will pick the
state up from here.

## Runs

| Started | Folder | Status | Outcome |
|---|---|---|---|
| 2026-08-07 | `2026-08-07-telegram-repair-bot--wip` | in progress | — |
```
...
Two things happen here: pick the file, write the skeleton between the `<!-- neopilot:start -->` markers. Announce the choice in one line inside the opening block, together with the mode — and do not wait for a reply.
...
`.neopilot/` is **not** ignored. It is the record of what was promised. The one exception is the last line — the running server's pid and log, which describe this machine at this minute and nothing else; §3 of `phases/0-instruments.md` adds it too, so a repo configured before 2026-08-19 gets it on the next flight.
...
`.neopilot/state.js` exists with `finishedAt` still `null` → this is a resume, not a new flight. (A run that finished is the third case at the top of this file, not this one — and at tier T0 there are no tickets to be unfinished, so `finishedAt` is the only reliable test.)
...
5. Continue from the frontier. Do not redo finished phases; do not re-ask answered questions.
   The `reviewers` handles in `state.js` belonged to the session that died — they cannot be reached from this one. Clear them back to `null`, and the next review spawns a fresh pair that rebuilds from `interfaces.md` (`phases/6-review.md`). The cross-ticket memory is the one thing a resume genuinely costs.
